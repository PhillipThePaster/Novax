...
bool ImGui::Checkbox(const char* label, bool* v){
    ...
    ImU32 check_color = *v ? GetColorU32((held && hovered) ? ImGuiCol_CheckMark : hovered ? ImGuiCol_CheckMark : ImGuiCol_CheckMark) : GetColorU32(ImGuiCol_FrameBg);

    if (*v) {

        window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(3, 3), check_bb.Max - ImVec2(3, 3), check_color, check_color, check_color, check_color);
...
void ImGui::RoundProgressBar(float fraction, float radius, float thickness, ImColor color, bool background, const char* customOverlay)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    ImVec2 center = bb.GetCenter();
    float progress = ImSaturate(fraction);

    if (background) {
        window->DrawList->AddCircleFilled(center, radius - style.FrameBorderSize, GetColorU32(ImGuiCol_FrameBg), 64);
    }

    if (progress > 0.0f) {
        const float circleStart = -IM_PI * 0.5f;
        const float circleEnd = circleStart + 2.0f * IM_PI * progress;

        window->DrawList->PathClear();
        window->DrawList->PathArcTo(center, radius - style.FrameBorderSize * 0.5f, circleStart, circleEnd, 64);
    ...
    {
        ImFormatString(overlay_buf, IM_ARRAYSIZE(overlay_buf), "%.0f%%", fraction * 100 + 0.01f);
        customOverlay = overlay_buf;
    }

    ImVec2 overlay_size = CalcTextSize(customOverlay, NULL);
    if (overlay_size.x > 0.0f)
    {
        ImVec2 text_pos(center.x - overlay_size.x * 0.5f, center.y - overlay_size.y * 0.2f);
        RenderTextColored(text_pos, customOverlay, color);
    }
}
...
bool ImGui::MultiCombo(const char* szLabel, unsigned int* pFlags, const char* const* arrItems, int nItemsCount)
{
    ...
    const char** vecActiveItems = (const char**)alloca(sizeof(const char*) * nItemsCount);
    int activeItemCount = 0;
    for (int i = 0; i < nItemsCount; i++)
    {
        if (*pFlags & (1 << i))
            vecActiveItems[activeItemCount++] = arrItems[i];
    }

    // Concatenate active items into a single string
    ...
    {
        const char* item = vecActiveItems[i];
        int itemLength = static_cast<int>(strlen(item));

        if (bufferOffset + itemLength + 2 < bufferSize) // Check if there's enough space
        {
            strcpy_s(strBuffer + bufferOffset, bufferSize - bufferOffset, item);
            bufferOffset += itemLength;

            if (i < activeItemCount - 1)
            {
                strcpy_s(strBuffer + bufferOffset, bufferSize - bufferOffset, ", ");
                bufferOffset += 2;
            }
        }
        else
        {
            // Handle buffer overflow (not enough space for all items)
            bufferOffset = bufferSize - 4;
            strcpy_s(strBuffer + bufferOffset, bufferSize - bufferOffset, "...");
            break;
        }
    }
    ...
    if (bufferOffset == 0)
    {
        strcpy_s(strBuffer, bufferSize, "none");
    }

    bool bValueChanged = false;
    if (BeginCombo(szLabel, strBuffer))
    {
        for (int i = 0; i < nItemsCount; i++)
        {
