...
std::map<int, std::string> CEntity::weaponNames = {
	...
	{9, "awp"}, {10, "famas"}, {11, "g3Sg1"}, {13, "galilar"}, {14, "m249"}, {17, "mac10"},
	{19, "p90"}, {23, "mp5sd"}, {24, "ump45"}, {25, "xm1014"}, {26, "bizon"}, {27, "mag7"},
	{28, "negev"}, {29, "sawedoff"}, {30, "tec9"}, {31, "zeus"}, {32, "p2000"}, {33, "mp7"},
	{34, "mp9"}, {35, "nova"}, {36, "p250"}, {38, "scar20"}, {39, "sg556"}, {40, "ssg08"},
	{42, "ct_knife"}, {43, "flashbang"}, {44, "hegrenade"}, {45, "smokegrenade"}, {46, "molotov"},
	{47, "decoy"}, {48, "incgrenade"}, {49, "c4"}, {16, "m4a1"}, {61, "usp"}, {60, "m4a1_silencer"},
	{63, "cz75a"}, {64, "revolver"}, {59, "t_knife"}
};
...
bool CEntity::UpdateController(const DWORD64& PlayerControllerAddress)
{
	...
	if (!this->Controller.GetMoney())
		return false;
	if (!this->Controller.GetHelmet())
		return false;
	if (!this->Controller.GetPing())
		return false;
...
bool PlayerController::GetPing() {
	return ProcessMgr.ReadMemory(Address + Offset::PlayerController.Ping, this->Ping);
}
...
bool PlayerController::GetHelmet() {
	return ProcessMgr.ReadMemory(Address + Offset::Pawn.m_bPawnHasHelmet, this->HasHelmet);
}

bool PlayerPawn::GetScoped() {
	return ProcessMgr.ReadMemory(Address + Offset::Pawn.isScoped, this->Scoped);
}
...
bool PlayerPawn::GetSpread()
{
	DWORD64 ClippingWeapon = 0;
	if (!ProcessMgr.ReadMemory<DWORD64>(Address + Offset::WeaponBaseData.ClippingWeapon, ClippingWeapon))
		return false;

	return GetDataAddressWithOffset<float>(ClippingWeapon, Offset::WeaponBaseData.m_flSpread, this->Spread);
}
...
bool PlayerPawn::IsPawnDefusing() {
	return ProcessMgr.ReadMemory(Address + Offset::Pawn.Defusing, this->Defusing);
}
...
bool PlayerPawn::GetFlashDuration(){
	return ProcessMgr.ReadMemory(Address + Offset::Grenade.flFlashDuration, this->FlashDuration);
}

bool PlayerPawn::GetVelocity()
{
	Vec3 Velocity;
	if (!ProcessMgr.ReadMemory(Address + Offset::Pawn.m_vecAbsVelocity, Velocity))
		return false;
	this->Velocity = sqrt(Velocity.x * Velocity.x + Velocity.y * Velocity.y);
	return true;
}
