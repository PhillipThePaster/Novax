...
public:
	...
	DWORD GetProcessID(std::string ProcessName){
		...
		memset(&ProcessInfoPE, 0, sizeof(PROCESSENTRY32)); 
		...
		USES_CONVERSION;
		do {
			if (ProcessInfoPE.szExeFile[0] != '\0' && strcmp(W2A(ProcessInfoPE.szExeFile), ProcessName.c_str()) == 0)
			{
	...
	HMODULE GetProcessModuleHandle(std::string ModuleName){
		...
		memset(&ModuleInfoPE, 0, sizeof(MODULEENTRY32));
		...
		USES_CONVERSION;
		do {
			const char* moduleA = W2A(ModuleInfoPE.szModule);
			const char* moduleNameA = ModuleName.c_str();
			if (moduleA != nullptr && moduleNameA != nullptr) {
				if (strcmp(moduleA, moduleNameA) == 0)
				{
					CloseHandle(hSnapshot);
