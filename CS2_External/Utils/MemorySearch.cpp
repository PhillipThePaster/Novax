...
std::vector<DWORD64> ProcessManager::SearchMemory(const std::string& Signature, DWORD64 StartAddress, DWORD64 EndAddress, int SearchNum)
{
	...
	while (VirtualQueryEx(hProcess, reinterpret_cast<LPCVOID>(StartAddress), &mbi, sizeof(mbi)) != 0)
	{
		...
		while (BlockSize >= BLOCKMAXSIZE)
		{
			...
			SearchMemoryBlock(MemoryBuffer, NextArray, SignatureArray, StartAddress + (static_cast<long long>(BLOCKMAXSIZE) * Count), BLOCKMAXSIZE, ResultArray);

			BlockSize -= BLOCKMAXSIZE;
			Count++;
		}

		SearchMemoryBlock(MemoryBuffer, NextArray, SignatureArray, StartAddress + (static_cast<long long>(BLOCKMAXSIZE) * Count), BlockSize, ResultArray);
